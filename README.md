...
- **4.2.0** — The calibration wave: the instrument gets fixed before the experiment, then frozen. Sensors moved to ground truth — commits enumerate `git log --since=<session anchor>` on every Bash observation (lexical `git commit` matching deleted; command text plays no part), test detection is command-position bound including the config override, health `commits`/`fix_ratio` share one git window (same-provenance; no git ⇒ null, never event counts), and a race-safe first-observation commit anchor (`eio_edits_since_last_commit`) makes Gate 1 immune to async duplicate observations. Deletions, each with its era: the healer (never repaired damage it didn't cause; health file is create-once + append-only now, lint-enforced), `state-io.sh` + the v3.7 migration chain + legacy carry-over reader + grandfathering (the v4.2 deletion calendar, executed after verifying zero bootstrap-era settings entries remained), `bootstrap-hooks.sh`, the `--native`/`native-hooks.ok` marker protocol, and `current-session.id` (the session id travels explicitly: boot context injection, pre-compact re-injection, skill/command arguments — no sid means skip-and-say-so, never guess). Real experimental conditions: `core` (recording + carry-over + blocking gates only — a core session provably emits zero advisory output) vs `lab` (core + the frozen adaptive tier); idle sessions write no health row; the statusline shows the eligible trend count it actually consumes. Provenance stamped per log (condition/plugin/repo/host). Baseline restore PROVEN (47/47 files hash-verified + an operational boot of `baseline-4.0.1` against the restored state). Docs honesty pass. The experiment protocol is pre-registered; the freeze begins at the cutoff.
- **4.0.1** — session_id containment (path-traversal sanitizer at the extraction boundary), healer predicate fix (exact v2 header signature — ended the heal/strip ping-pong), UserPromptSubmit additionalContext emission, codex-review detection requires a prompt-bearing subcommand, test-runner python3 executability probe.
